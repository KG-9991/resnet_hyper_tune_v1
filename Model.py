...
class Cifar(nn.Module):
    ...
    def test_or_validate(self, x, y, checkpoint_num_list):
        ...
        for checkpoint_num in checkpoint_num_list:
            checkpointfile = os.path.join(self.config.modeldir+'ver1-%d'%(self.config.model_number), 'model-%d.ckpt'%(checkpoint_num))
    ...
    def save(self, epoch):
        checkpoint_path = os.path.join(self.config.modeldir+'ver1-%d'%(self.config.model_number), 'model-%d.ckpt'%(epoch))
        os.makedirs(self.config.modeldir+'ver1-%d'%(self.config.model_number), exist_ok=True)
